...
class LoginController extends Controller
{
    ...
    */
	...
	public function handleGoogleCallback()
    {
        try {
			...
			if(empty($getUser[0])){
				...
				if ($finduser) {
					User::where('id',$finduser->id)->update(['socialite_token' =>$user->token,'new_user' =>1,'user_image'=>!empty($user->user['picture']) ? $user->user['picture'] : '']);
					Auth::login($finduser);
					...
					if(empty($findEmail)){
						$newUser = new User;
					$newUser->name = !empty($user->name) ? $user->name : '';
					...
					return redirect('/dashboard');
						...
						return redirect('/')->with('error','This email id is already exist');
					}
...

    {
        ...
        return Socialite::driver('facebook')->fields(['first_name', 'last_name', 'email', 'gender', 'birthday'])->scopes(['user_friends'])->redirect();
    ...
    public function handleOutlookCallback(Request $request)
    {
        ...
        if (isset($authCode)) {
            ...
            $oauthClient = new \League\OAuth2\Client\Provider\GenericProvider([
                'clientId'                => !empty($objApiOauthIdCredential->api_value) ? $objApiOauthIdCredential->api_value : '',
            'clientSecret'            => !empty($objApiOauthAppPasswordCredential->api_value) ? $objApiOauthAppPasswordCredential->api_value : '',
            'redirectUri'             => !empty($objApiOauthRedirectUrlCredential->api_value) ? $objApiOauthRedirectUrlCredential->api_value : '',
            'urlAuthorize'            => $objApiOauthAuthorityCredential->api_value . $objApiOauthAuthorizeEndpointCredential->api_value,
            'urlAccessToken'          => $objApiOauthAuthorityCredential->api_value . $objApiOauthTokenEndpointCredential->api_value,
            'urlResourceOwnerDetails' => '',
            'scopes'                  => $objApiOauthScopesCredential->api_value
            ]);
            ...
            try {
				...
				$getUser = User::where('email',$getEmail)->get();
			    if(empty($getUser[0])){
					$finduser = User::where('outlook_id', $user->getId())->first();
					if ($finduser) {
						$updateUser = User::where('id', $finduser->id)->update(['socialite_token' => $accessToken->getToken()]);
						Auth::login($finduser);
						return redirect('/dashboard');
					} else {
		...
		if ($user !== Null) {
			...
			if (Hash::check($password, $user->password)) {
				if (Auth::attempt($credentials)) {
					...
					return response()->json(array('status' => 200, 'message' => 'Login Successfully!','userType'=>$userType));
				}else{
					return redirect('/');
				}
			}else{
				 return response()->json(array('status' => 201, 'message' => 'Incorrect Password!'));
			}
		 }else{
			 return response()->json(array('status' => 201, 'message' => 'Your email id is not verified.<a href="javascript:;" data-user-id="'.$user->id.'" class="verify-user"> Click here</a> to resend the verification mail'));
		 }
		}else{
			return response()->json(array('status' => 201, 'message' => 'Email Id Not Exist!'));
		}
	}
