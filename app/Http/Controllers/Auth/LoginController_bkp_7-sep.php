...
class LoginController extends Controller
{
    ...
    */
    ...
    public function postLogin(Request $request)
    {
        ...
        if (($request->input('type_user') == $user->user_type))
        {
            if (!empty($getBusiness) || ($user->user_type == 1))
            {
                if ($user !== Null)
                {
                    if (($user->step == 4) || ($user->step == Null))
                    {
                    ...
                    {
                        if (!empty($user->step) && ($user->step == 2))
                        {
                            return response()
                                ->json(array(
                                'status' => 202,
                            ...
                            return redirect('business/user/signup/' . $user->id);
                        }
                        elseif (!empty($user->step) && ($user->step == 3))
                        {
                            return response()
                                ->json(array(
                                'status' => 202,
            ...
            else
            {
                return response()
                    ->json(array(
                    'status' => 201,
                    'message' => 'this user has not any business'
                ));
            }
        }
        else
