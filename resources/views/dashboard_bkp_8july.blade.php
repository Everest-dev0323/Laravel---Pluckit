...
<div id="content">
  <section class="section">
	<div class="container">
	  <!-- Client Profile -->
	  <div class="client-profile pb-4">
		<div class="column">
		  <div class="media mb-3">
		  @if(!empty($getUser->user_image))
			<img class="img-fluid mr-3" src="{{ $getUser->user_image }}" alt="{{ Auth::user()->name }}">
		  @else
			<img class="img-fluid mr-3" src="{{ ('/public') }}/front-assets/images/default-profile.jpg" alt="Avatar">
		  @endif
				  ...
				  {{ csrf_field()}}
					<div class="upload-profile-image">
					  ...
					  <!-- <div class="file-upload mr-sm-2">-->
						  <input id="profileimage" type="file" name="profile_image" class="inputfile" onChange="preview_image(event,'profile_image')" style="display:none">
					...
					<div class="form-group">
					  <label for="profile-text">Edit profile text</label>
					...
					<div class="form-group">
					  ...
					  <input type="text" name="firstname" id="fullname" class="form-control @error('firstname') is-invalid @enderror"
						placeholder="Enter First Name" value="{{ !empty($getUser->name) ? $getUser->name : ''}}" required>
					@error('firstname')
					...
					<div class="form-group">
					  ...
					  <input type="text" name="lastname" id="lastname" class="form-control @error('lastname') is-invalid @enderror" placeholder="Enter Last Name" value="{{ !empty($getUser->lastname) ? $getUser->lastname : ''}}" required>
					  @error('lastname')
						<span class="invalid-feedback" role="alert">
							<strong>{{ $message }}</strong>
						</span>
					...
					<div class="form-group">
					  ...
					  <input type="tel" name="phone_no" id="tel" class="form-control" placeholder="+61" data-parsley-type="number" value="{{ !empty($getUser->mobile_no) ? $getUser->mobile_no : ''}}">
					</div>
					<div class="form-group">
					...
					<div class="form-group">
					  ...
					  <input type="text" name="address" id="street-address" class="form-control"
						placeholder="17 St Lucia PL" value="{{ !empty($getUser->address) ? $getUser->address : ''}}">
					</div>
					<div class="form-group">
					  <label for="suburb">Suburb</label>
					  <input type="text" name="suburb" id="suburb" class="form-control"
						placeholder="Bonny Hills" value="{{ !empty($getUser->suburb) ? $getUser->suburb : ''}}">
					</div>
					<div class="form-group">
					  <label for="suburb">State</label>
					  <input type="text" name="state" id="state" class="form-control"
						placeholder="NSW" value="{{ !empty($getUser->state) ? $getUser->state : ''}}">
					</div>
					<div class="form-group">
					  <label for="postcode">Post code</label>
					  <input type="text" name="postcode" id="postcode" class="form-control" placeholder="2445" data-parsley-type="number" value="{{ !empty($getUser->postcode) ? $getUser->postcode : ''}}">
					</div>
			       ...
			       @php $getObjUser = \App\User::where('id',$review->user_id)->first(); @endphp
					  <div class="box bg-gray-light box-shadow">
						<div class="media">
						 ...
						 <img class="img-fluid" src="{{ (!empty($getObjUser->user_image))? $getObjUser->user_image: url('/public/front-assets/images/avatar-default.png') }}" alt="{{ $review->user_name }}">
						 </figure>
						  <div class="media-body">
			  ...
			  <div class="tab-pane fade {{ (!empty($input['step']) || $input['step']==5) ? 'show' : '' }} {{ (!empty($input['step']) || $input['step']==5) ? 'active' : '' }}" id="pills-help" role="tabpanel" aria-labelledby="pills-help-tab">
				...
				<div class="box bg-gray-light box-shadow p-md-4 mt-4">
				  ...
				  @csrf
					...
					<div class="form-group">
					  <label class="file">
						<input type="file" id="file" name="attach_file" aria-label="File browser example">
		...
		<aside id="sidebar" class="col-lg-4 pl-lg-5">
		  ...
		  <div class="box bg-gray-light box-shadow">
			<ul class="list-unstyled import-contacts">
			  <li><a href="#" class="facebook"><span class="icon2"><i class="fab fa-facebook-f"></i></span>Import
				  From Facebook</a></li>
			@php
			...
			<p class="text-danger mb-0">Import your friend list and find top <br /> Tradies recommended by your friends</p>
		  </div>
		</aside>
	  </div>
	</div>
...
<script>
    ...
    if(!empty($getNewUser)) { ?>
            ...
            swalWithBootstrapButtons.fire({
                    ...
                    text: "Select Import Contact or Ignore",
                    showCancelButton: false,
                    showConfirmButton: false,
                    confirmButtonText: 'Custom Project',
                    cancelButtonText: 'No, cancel!',
                    reverseButtons: false,
                    footer: '<a href="{{ route('google.import') }}" class="btn btn-success">Import contact</a>'+
                             '<a href="{{ route('ignore-google-import') }}" class="btn btn-primary">Ignore</a>'
                }).then((result) => {
