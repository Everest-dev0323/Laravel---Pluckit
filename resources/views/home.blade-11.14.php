  ...
  <div id="content" class="pt-0">
    ...
    <section class="section three-columns">
      <div class="container">
        <div class="section-head">
          ...
          <h5>"You're not alone on here. 89% of people now check recommendations before buying" - Trustpilot 2020.</h5>
        ...
        <div class="col-sm-6 col-md-6 col-lg-4">
            ...
            <figure class="figure m-4 text-center divider">
              ...
              <div class="row justify-content-center align-items-center">
                ...
                <div class="col-7 pl-0 category text-right">
                  <a href="{{ url('view-categories')}}">{{ !empty($category->category_name) ? $category->category_name : '---' }}</a>
                </div>
                </div>
              </div>
            </div>
    ...
    <section class="section categories">
        ...
        <div class="row mb-lg-5">
		 ...
		 @php
		  $varRating = 0;
		    ...
		    $getSubcategory = \App\Category::where('pluckit_parent_id',$category->id)->get();
        $getSubcategoryCount = count($getSubcategory);
				if(!empty($getSubcategory[0])){
					foreach($getSubcategory as $subcategory){
					 $arrSubcategoryName[] = $subcategory->alias;
		 ...
		 @endphp
		 @php
		    if(!empty($arrSubcategoryName)){
			 $categoryRating = \App\BusinessListingCategory::whereIn('business_listing_categories.business_category_alias',$arrSubcategoryName)->get(['business_listing_categories.business_id']);
		    }else{
			...
			if(!empty($categoryRating[0])){
				...
				foreach($categoryRating as $ratingCategory){
					$businessReview = \App\BusinessListing::where('business_listings.id',$ratingCategory->business_id);
					$businessReview = $businessReview->join('business_listing_categories as tlc','tlc.business_id','=','business_listings.id');
					$businessReview = $businessReview->first('business_listings.*');
      ...
      if($getSubcategoryCount > 0) {
          $varAverageViews = round($varReviewCount/$getSubcategoryCount);
        }
        else {
          $varAverageViews = 0;
        }
                  ...
                  <div class="col-8 pr-0">
                    ...
                    <div class="rating d-flex justify-content-start align-items-center">
                      ...
                      <div class="reviews ml-sm-2 ml-1">
                        <a href="#">({{ $varAverageViews }} recommendations )</a>
                      </div>
                    </div>
                  </div>
