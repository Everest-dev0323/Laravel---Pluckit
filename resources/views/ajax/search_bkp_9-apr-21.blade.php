...
@php
   ...
   $objFriendRecommendationList = \App\BusinessListReview::whereIn('user_id',$arrFriendRecommendation)->where('status',1)->where('business_id',$business['business_review_count'])->orderBy('review_time_created','desc')->limit(3)->get();
   $objFriendRecommendationCount = \App\BusinessListReview::whereIn('user_id',$arrFriendRecommendation)->where('status',1)->where('business_id',$business['business_review_count'])->get()->count();
 ...
 @endphp
  ...
  <div class="filter-item">
	<div class="service-box">
	  ...
	  <div class="service-content">
                    <div class="left mr-sm-3">
					  ...
					  <p>
                      @if($userId == 0)
					  <a><b><span class="btn btn-link btn-login" data-toggle="modal" data-target="#loginModal">Login</span> to see your friends recommendations</b></a>

					  @else
						  @if($objFriendRecommendationCount != 0)
							<a href="javascript:;"  onClick="clickFriendRecommendation({{ $business['id'] }})">{{ $objFriendRecommendationCount }} or more people you might know refer or recommend this business</a>
						  @endif
					  @endif
					  </p>
	...
	<div class="top-search d-none" id="friend-recommendation-{{ $business['id'] }}" data-status=''>
	  <!-- review-list -->
	  <p class="lead">{{ ($objFriendRecommendationCount <= 3) ? $objFriendRecommendationCount : '3' }} or more people you might know refer or recommend this business</p>
