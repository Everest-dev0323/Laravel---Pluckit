...
@php
   ...
   $objFriendRecommendationCount = \App\BusinessListReview::where('user_id','!=',$userId)->where('status',1)->where('business_id',$business['id'])->whereIn('user_id',$arrFriendRecommendation)->whereIn('review_type',$pluckitType)->orderBy('review_time_created','desc')->groupBy('user_id')->get()->count();
